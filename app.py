...
# ─── Helpers ────────────────────────────────────────────────────────
# app.py lives at the PROJECT ROOT, so all folders are siblings, not ../
...
def engineer_features(df):
    ...
    # Fixed mean=50 so single-row prediction does not divide by itself
    df['training_effectiveness'] = df['training_hours'] / 50.0
    return df
...
def predict_employee(emp_dict):
    ...
    # Pandas 2.x safe: cast to int Series to avoid StringDtype conflict
    ...
    if uploaded:
        ...
        if st.button("⚡ Run Batch Prediction"):
            try:
                model, scaler, le_target, le_dict, features = load_model_artifacts()
                df_proc = df_up.copy()
                # Pandas 2.x safe encoding
                for col, le in le_dict.items():
                    if col in df_proc.columns:
                        encoded = le.transform(df_proc[col].astype(str)).astype(int)
